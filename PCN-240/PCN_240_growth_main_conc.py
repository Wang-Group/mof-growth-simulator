...
H2O_DMF_RATIO = 0
Capping_agent_conc = 200
Linker_conc = 34.57984361265395
Total_steps = 1000000000000000
current_folder = '/mnt/syh/MOF_growth_data/Fe_Co3_BDC_OH/Fe_Co3_BDC_OH_260206/Stage_1/Zr_22.9154705859894_FA_200_L_34.57984361265395_Ratio_0_Step_1e15_Index_0_SC_0.789387907185137_KC_1.32975172557788_Nmax_500_2026-02-06_21_28_12/'
BUMPING_THRESHOLD = 2
pkl_path = None
max_entities = 500
output_inter = 50
...
from PCN_240_Assembly_Large_Correction_conc import *
import numpy as np
...
Entropy_correction_table = [entropy_assembly(ENTROPY_GAIN, i) for i in range(30000)]
 # the concentration of Zr in the unit of mM
Zr6_conc = Zr_conc*ZR6_PERCENTAGE/3
Zr12_conc = Zr_conc*(1-ZR6_PERCENTAGE)/12
Zr6_conc_0 = Linker_conc*2/6
...
step = 0
entities_number = []
last_by_n_folder = current_folder + '/last_by_n/'
os.makedirs(last_by_n_folder, exist_ok=True)
...
for cycle in range(Total_steps + 1):
    ...
    if n > 0:
        # 只保留“最后一次 n 的结构”：固定文件名，重复出现就覆盖
        mol2_path = os.path.join(last_by_n_folder, f"assembly_last_entity_number_{n}.mol2")
        pkl_path_n = os.path.join(last_by_n_folder, f"assembly_last_entity_number_{n}.pkl")

        assembly.get_mol2_file(mol2_path)

        rebuild_after = (cycle < Total_steps)  # 你原来逻辑保持不变
