...
DEFAULT_SEED_MOL2 = CASE_DIR / "references" / "UiO-66_R3_Mixeddef_0.40_seed800.mol2"
DEFAULT_MILESTONE_MOL2 = CASE_DIR / "references" / "assembly_2026-04-03_15-42-13_entity_number_2800.mol2"
DEFAULT_FINAL_MOL2 = CASE_DIR / "assembly_final_entity_number_100001.mol2"
DEFAULT_PNG = CASE_DIR / "seed_initial_final_structure_triptych.png"
DEFAULT_SVG = CASE_DIR / "seed_initial_final_structure_triptych.svg"
...
def load_series(label, path: Path):
...
def make_plot(series, png_out: Path, svg_out: Path):
    ...
    fig.suptitle("Recovered Missing_BDC_Zr6 structures", fontsize=18, color="#1f2937")
    fig.text(
        0.5,
        0.94,
        (
            "Left to right: mixed-defect seed, tracked repaired nucleus, and the final "
            "Zr6-only growth endpoint under the same fixed chemistry."
        ),
        ha="center",
        fontsize=10,
        color="#475569",
...
def parse_args():
    parser = argparse.ArgumentParser(description="Create the recovered Missing_BDC_Zr6 structure triptych.")
    parser.add_argument("--seed-mol2", type=Path, default=DEFAULT_SEED_MOL2)
    parser.add_argument("--milestone-mol2", type=Path, default=DEFAULT_MILESTONE_MOL2)
...
def main():
    ...
    series = [
        ...
        load_series("Tracked nucleus", args.milestone_mol2),
