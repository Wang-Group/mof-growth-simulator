...
def parse_args():
    ...
    parser.add_argument(
        ...
        choices=["time", "step"],
        default="time",
        help="How to align runs before averaging. 'time' is usually the physically sensible choice.",
    )
    parser.add_argument(
        "--mean-grid-points",
        type=int,
        default=400,
        help="Number of points in the common time grid when --mean-alignment=time.",
    )
...
def aggregate_mean_trace_by_step(run_payloads):
    ...
    for step in sorted(values_by_step):
        ...
        mean_rows.append(
            {
                "step": int(step),
                "sim_time_seconds": float(np.mean([row["sim_time_seconds"] for row in rows])),
...
def sample_trace_on_time_grid(trace_rows, time_grid):
    times = np.asarray([row["sim_time_seconds"] for row in trace_rows], dtype=float)
    z6 = np.asarray([row["Zr6_AA"] for row in trace_rows], dtype=float)
    z12 = np.asarray([row["Zr12_AA"] for row in trace_rows], dtype=float)
    bdc = np.asarray([row["BDC"] for row in trace_rows], dtype=float)
    total = np.asarray([row["total_entities"] for row in trace_rows], dtype=float)

    # Counts are piecewise-constant between stochastic events, so use hold-last-value
    # interpolation instead of linear blending.
    idx = np.searchsorted(times, time_grid, side="right") - 1
    idx = np.clip(idx, 0, len(trace_rows) - 1)
    return {
        "Zr6_AA": z6[idx],
        "Zr12_AA": z12[idx],
        "BDC": bdc[idx],
        "total_entities": total[idx],
    }


def aggregate_mean_trace_by_time(run_payloads, grid_points):
    if not run_payloads:
        return []

    max_time = max(run["traces"][-1]["sim_time_seconds"] for run in run_payloads)
    if max_time <= 0.0:
        time_grid = np.asarray([0.0], dtype=float)
    else:
        point_count = max(2, int(grid_points))
        time_grid = np.linspace(0.0, max_time, num=point_count, dtype=float)

    sampled_runs = [sample_trace_on_time_grid(run["traces"], time_grid) for run in run_payloads]

    mean_rows = []
    for grid_index, sim_time_seconds in enumerate(time_grid):
        mean_rows.append(
            {
                "step": int(grid_index),
                "sim_time_seconds": float(sim_time_seconds),
                "Zr6_AA_mean": float(np.mean([run["Zr6_AA"][grid_index] for run in sampled_runs])),
                "Zr12_AA_mean": float(np.mean([run["Zr12_AA"][grid_index] for run in sampled_runs])),
                "BDC_mean": float(np.mean([run["BDC"][grid_index] for run in sampled_runs])),
                "total_entities_mean": float(np.mean([run["total_entities"][grid_index] for run in sampled_runs])),
                "replicate_count": int(len(sampled_runs)),
            }
        )
    return mean_rows


def aggregate_mean_trace(run_payloads, *, alignment, grid_points):
    if alignment == "step":
        return aggregate_mean_trace_by_step(run_payloads)
    return aggregate_mean_trace_by_time(run_payloads, grid_points)
...
def make_svg_plot(run_payloads, mean_trace, svg_path: Path, *, mean_alignment: str):
    species_meta = {
    ...
    parts = [
        ...
        + escape(f"thin lines = individual replays; thick lines = {mean_alignment}-aligned mean trajectory")
        + "</text>",
    ]
    ...
    for species, meta in species_meta.items():
        points = [
            (x_px(row["sim_time_seconds"]), y_px(row[f"{species}_mean"]))
...
def main():
    ...
    mean_trace = aggregate_mean_trace(
        replay_runs,
        alignment=args.mean_alignment,
        grid_points=args.mean_grid_points,
    )

    svg_path = output_dir / f"{basename}.svg"
    json_path = output_dir / f"{basename}.json"

    make_svg_plot(replay_runs, mean_trace, svg_path, mean_alignment=args.mean_alignment)

    json_payload = {
        "summary_json": summary_path.as_posix(),
        "snapshot_every_steps": args.snapshot_every_steps,
        "mean_alignment": args.mean_alignment,
        "mean_grid_points": args.mean_grid_points,
        "candidate": candidate,
        "config": config,
