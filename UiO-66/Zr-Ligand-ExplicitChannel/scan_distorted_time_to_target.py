...
def build_case_inputs(job):
    ...
    prebound_zr_bdc_addition_activity = min(
        max(
            float(
                distorted_state.get(
                    "prebound_zr6_cluster_fraction",
                    distorted_state.get("prebound_zr_bdc_fraction", 0.0),
                )
            ),
            0.0,
        ),
    ...
    return {
        ...
        "prebound_zr_bdc_addition_activity": prebound_zr_bdc_addition_activity,
    }
...
def run_case(job):
    ...
    for step in range(int(job["max_steps"])):
        ...
        flag, selected_carboxylate, total_growth_rate, selected_pair, addition_mode = assembly.next_thing_to_do(
            ...
            prebound_zr_bdc_addition_activity=case_inputs["prebound_zr_bdc_addition_activity"],
        )
